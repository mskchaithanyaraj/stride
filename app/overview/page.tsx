...
export default function Overview() {
  ...
  const handleGuestMode = () => {
    ...
    toast("We are working on the feature :)", {
      icon: "🚧",
      duration: 3000,
    });
    setTimeout(() => setIsGuestLoading(false), 1000);
  };
    ...
    <RouteGuard requireAuth={false} redirectTo="/home">
      <div className="min-h-screen bg-gradient-to-br from-[var(--background)] via-[var(--surface)] to-[var(--background)] text-[var(--foreground)] flex items-center justify-center px-6">
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center">
          ...
          {/* Auth Actions (stacked vertically) */}
          <div className="mt-2 flex flex-col items-center gap-4">
            <button
              onClick={handleGoogleSignIn}
              disabled={isGoogleLoading}
              className="w-full max-w-md inline-flex items-center justify-center px-8 py-4 bg-white dark:bg-[var(--surface)] text-gray-800 dark:text-[var(--foreground)] rounded-full font-semibold text-lg border-2 border-[var(--border)] hover:shadow-2xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isGoogleLoading ? (
            ...
            <button
              ...
              className="w-full max-w-md inline-flex items-center justify-center px-8 py-3 bg-transparent text-[var(--foreground)] rounded-full font-medium text-base border-2 border-[var(--border)] hover:bg-[var(--hover)] hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isGuestLoading ? (
                <div className="w-5 h-5 border-2 border-[var(--foreground)] border-t-transparent rounded-full animate-spin mr-2"></div>
              ) : null}
              {isGuestLoading ? "Loading..." : "Use as Guest"}
            </button>
          </div>
